...
| 顺序线性结构（该类线性结构是一直维持着元素顺序） | O(n) |         O(1)         |
...
​	二叉堆（Binary Heap）就是一颗完全二叉树，注意<u>完全二叉树和满二叉树是不一样的</u>，满二叉树就是满的树（除了叶子节点，其他节点的左右孩子都不为空），完全二叉树不一定是满的，不满的那一层一定是从左边开始接的。<u>二叉树和前面学习的二叉搜索树也是不一样的</u>。
